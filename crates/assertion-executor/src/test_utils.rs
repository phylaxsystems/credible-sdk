...
fn cached_artifact_bytecodes(input: &str) -> ArtifactBytecodes {
    // Benchmarks repeatedly request the same small artifact set. Keep a read-mostly cache so
    // they avoid reopening JSON files and re-decoding hex on every setup iteration.
