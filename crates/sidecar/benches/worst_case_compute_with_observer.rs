//! `worst-case-compute-with-observer` benchmark.
//! Send 100 transactions targeting the same adopter (500 total assertion executions)
//! with the transaction observer enabled.
...
fn run_benchmark_for_driver<T, SetupFn, Fut>(
    ...
    SetupFn: Fn(AssertionStore, IncidentReportSender) -> Fut + Copy + Send + 'static,
    Fut: Future<Output = Result<LocalInstance<T>, String>>,
{
...
fn main() {
    ...
    let transport = std::env::var("SIDECAR_BENCH_TRANSPORT")
        ...
        .unwrap_or_default();
    let bench_label = format!("worst_case_compute_with_observer ({})", transport.as_str());
    tracing::info!(
        "Running observer-enabled benchmark with `{}` transport",
        transport.as_str()
    );
    ...
    match transport {
        BenchTransport::Grpc => {
            run_benchmark_for_driver::<LocalInstanceGrpcDriver, _, _>(
                ...
                Arc::clone(&bytecodes),
                Arc::clone(&adopters),
                LocalInstanceGrpcDriver::new_with_store_and_incident_sender,
            )
        }
        BenchTransport::Http => {
            run_benchmark_for_driver::<LocalInstanceHttpDriver, _, _>(
                ...
                Arc::clone(&bytecodes),
                Arc::clone(&adopters),
                LocalInstanceHttpDriver::new_with_store_and_incident_sender,
            )
        }
        BenchTransport::Mock => {
