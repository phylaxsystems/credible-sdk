...
impl Sources {
    ...
    pub fn set_block_number(&self, block_number: u64) {
        ...
        for source in &self.sources {
            source.update_latest_head(minimum_synced_block_number);
        }
    }

    /// Resets the `reset_required_head` to the current `block_number`.
    ///
    /// This method updates the `required_head` to match the value of the
    ...
    pub fn reset_required_head(&self, required_head: u64) {
        self.metrics.increase_reset_required_head_counter();
        self.required_head.store(required_head, Ordering::Relaxed);
    }
    ...
    fn get_minimum_synced_block_number(&self) -> u64 {
        ...
        let required_head = self.required_head.load(Ordering::Acquire);
