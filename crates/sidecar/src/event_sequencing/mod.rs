...
impl EventSequencing {
        ...
        loop {
            match self.tx_receiver.try_recv() {
                ...
                Err(crossbeam::channel::TryRecvError::Empty) => {
                    // Sleep briefly to avoid busy-waiting and excessive CPU usage
    ...
    fn process_event(&mut self, event: TxQueueContents) -> Result<(), EventSequencingError> {
        ...
        // Ignore events older than the current head
    ...
    fn handle_sequential_event(
        ...
        if *previous_sent_event == previous_event {
            ...
            // Restore the previous sent event
            // self.restore_sent_event(block_number, event_iteration_id, previous_sent_event)?;
    ...
    fn send_event_recursive(
        ...
        if is_commit_head {
            self.handle_commit_head_completion(block_number)?;
        }
    ...
    fn handle_commit_head_completion(
        ...
        self.context.remove(&block_number);
    ...
    fn try_start_next_block(&mut self) -> Result<(), EventSequencingError> {
        ...
        // Look for NewIteration events that have no predecessors (root events)
        // We need to find events that aren't in anyone's dependency list
        let all_new_iterations: Vec<(EventMetadata, TxQueueContents)> = context
            .dependency_graph
            .values_mut()
            .flat_map(|deps| {
                deps.extract_if(|event_metadata, _| event_metadata.is_new_iteration())
                    .collect::<Vec<_>>()
            })
            .collect();
        ...
        for (meta, event) in all_new_iterations {
            ...
            self.send_event_recursive(event, &meta)?;
        }
