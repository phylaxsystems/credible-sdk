...
async fn test_cumulative_state_with_different_accounts() -> Result<()> {
    ...
    let update0 = create_test_update(0, B256::repeat_byte(0xAA), addr_0x11, 1000, 5, vec![], None);
...
async fn test_cumulative_state_with_account_updates() -> Result<()> {
    ...
    let update0 = create_test_update(0, B256::repeat_byte(0xBB), address, 1000, 0, vec![], None);
    writer.commit_block(update0).await?;

    // Block 1: Same account, balance increases to 1500, nonce to 1
    let update1 = create_test_update(1, B256::repeat_byte(0xBB), address, 1500, 1, vec![], None);
    writer.commit_block(update1).await?;

    // Block 2: Same account, balance decreases to 1200, nonce to 2
    let update2 = create_test_update(2, B256::repeat_byte(0xBB), address, 1200, 2, vec![], None);
    writer.commit_block(update2).await?;
    ...
    let update3 = create_test_update(3, B256::repeat_byte(0xBB), address, 2000, 3, vec![], None);
    writer.commit_block(update3).await?;
...
async fn test_account_deletion_persists() -> Result<()> {
    ...
    let update0 = create_test_update(0, B256::repeat_byte(0xBB), address, 1000, 5, vec![], None);
    writer.commit_block(update0).await?;
