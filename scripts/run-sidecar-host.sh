...
export SIDECAR_STATE_SOURCES="${SIDECAR_STATE_SOURCES:-$(printf '[{"type":"mdbx","mdbx_path":"%s","depth":%s},{"type":"eth-rpc","ws_url":"%s","http_url":"%s"}]' "${SIDECAR_STATE_INTEGRATED_WORKER_MDBX_PATH}" "${SIDECAR_STATE_INTEGRATED_WORKER_DEPTH}" "${SIDECAR_STATE_ETH_RPC_SOURCE_WS_URL}" "${SIDECAR_STATE_ETH_RPC_SOURCE_HTTP_URL}")}"
