...
DEFAULT_STATE_WORKER_GENESIS_PATH="docker/maru-besu-sidecar/config/l2-genesis-initialization/genesis-besu.json"
...
STATE_WORKER_GENESIS_PATH="${SIDECAR_STATE_WORKER_FILE_TO_GENESIS:-${DEFAULT_STATE_WORKER_GENESIS_PATH}}"
...
export SIDECAR_STATE_WORKER_WS_URL="${SIDECAR_STATE_WORKER_WS_URL:-ws://127.0.0.1:8546}"
export SIDECAR_STATE_WORKER_MDBX_PATH="${STATE_WORKER_MDBX_PATH}"
export SIDECAR_STATE_WORKER_FILE_TO_GENESIS="${STATE_WORKER_GENESIS_PATH}"

if [[ ! -f "${ROOT_DIR}/${SIDECAR_STATE_WORKER_FILE_TO_GENESIS}" && ! -f "${SIDECAR_STATE_WORKER_FILE_TO_GENESIS}" ]]; then
  echo "missing genesis file: ${SIDECAR_STATE_WORKER_FILE_TO_GENESIS}" >&2
  exit 1
fi
