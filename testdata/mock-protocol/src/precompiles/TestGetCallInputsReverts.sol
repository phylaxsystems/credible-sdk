...
contract TestGetCallInputsReverts is Assertion, Test {
    function testGetCallInputsReverts() external view {
        PhEvm.CallInputs[] memory callInputs = ph.getCallInputs(address(TARGET), Target.unhandledRevert.selector);
        ...
        callInputs = ph.getCallInputs(address(TARGET.reverts()), "");
        require(callInputs.length == 1, "callInputs.length != 1");

        callInput = callInputs[0];
        require(callInput.target_address == address(TARGET.reverts()), "callInput.target_address != target");
